...
类型定义（也称为 typedef）是 bbpbcn 用来存储消息及其字段元数据的数据结构。它包含每个字段的确切类型以及字段名称等其他元数据。

## 解码

解码消息时，typedef 是**可选**的。如果提供了字段类型和名称，解码时会使用它们，但如果没有 typedef 或 typedef 不包含某字段的元数据，bbpbcn 会根据该字段的 "wire type"（见下方的 Wire Types）选择默认类型。

解码时 bbpbcn 猜测的默认类型可能与实际类型不匹配，仅是一种尽力而为的猜测。用户可能需要使用 typedef 将类型改为正确的值。
...
type 字段是必需字段，应包含该字段的 bbpbcn 类型。这些类型大致映射到 protobuf 类型，但可能不完全一致。
...
最新的类型列表也可在 [type_maps.py](/lib/bbpbcn/lib/types/type_maps.py) 中找到。
...
这意味着使用相同消息类型编码的两个 protobuf 消息可能包含完全不同类型的内嵌消息，而 bbpbcn 无法确定使用了哪种类型或预测未来消息的类型。

处理这些消息的"技术上正确"的方法是将类型设置为 `bytes` 并不解码内嵌消息，但这不太实用。相反，如果 bbpbcn 对某个字段已经有 `message_typedef`，并且该 `message_typedef` 无法有效解码该字段的某个实例，它将尝试使用 `alt_typedefs` 中的任何类型定义。如果 `alt_typedefs` 中没有可用的类型定义，bbpbcn 会向 `alt_typedefs` 中添加新的类型定义。
...
在极少数情况下，`alt_typedef` 字典可以包含一个类型字符串（如 `string` 或 `bytes`）而不是类型定义。这仅当创建 typedef 时字段包含可解码消息，但 bbpbcn 在后续运行时未能识别为有效的 protobuf 消息时才会发生。

### `field_order`

Field order 包含 protobuf 子消息中字段被解码的顺序。这有助于 bbpbcn 避免意外改变被错误解码为消息的字节字段，但对于合法的 protobuf 消息通常不需要。
...
- Start/End Group：本身不是字段，而是将一组字段用标签包裹在一起。已弃用，建议使用子消息，bbpbcn 不支持

基于 wire type，bbpbcn 可以猜测每个字段的正确类型，同时确保字段可以重新编码为有效的 protobuf。
...
- `uint` — 无符号整数，可以表示比 `int` 更大的值
    - 如果 bbpbcn 解码出负数而它们应该始终为正，可选择此类型
...
bbpbcn 中没有专门的布尔类型，但布尔值被编码为 varint，值为 0 表示 False，1 表示 True。
...
bbpbcn 不会为 length delimited 字段使用单一默认类型，而是首先尝试将字段解码为 `message`，如果解码失败则回退到 `string`，如果 `string` 也失败则最终回退到 `bytes`。
...
- `packed_*` — Packed 字段是一种更高效的表示重复值（如列表或数组）的机制，通过在编码时移除重复的 tag（包含字段编号和 wire type 的 varint）来实现。
    - bbpbcn 支持以下 packed 类型：
        ...
        - `packed_sfixed64`
        - `packed_double`
    - bbpbcn 没有检测 packed 字段的机制，这些类型必须由用户明确设置。
...
wire type 的默认类型可以通过向解码器提供 `Config` 对象并使用 `default_types` 字典（位于 [config.py](/lib/bbpbcn/lib/config.py#L49)）覆盖默认类型来修改。字典的键是 wire type，可以在 [wiretypes.py](/lib/bbpbcn/lib/types/wiretypes.py) 中找到，并与 <https://protobuf.dev/programming-guides/encoding/#structure> 中的 wire type 匹配。
...
`Config` 中的 `default_types` 字段不用于 length delimited 类型，因为 length delimited 字段有特殊的回退逻辑。对于这些字段，你可以通过更改 `Config` 对象上的 `default_binary_type`（位于 [/lib/bbpbcn/lib/config.py#L45](/lib/bbpbcn/lib/config.py#L45)）来替换 `bytes` 回退类型。但与默认类型不同，解码器在回退到 `default_binary_type` 之前仍会尝试将字段解码为消息或字符串。
...
"name" 字段将用作解码消息中的字典键。编码器接受原始字段编号或 typedef 的 "name" 字段中指定的名称。字段名称需要唯一，应为字母数字加下划线，但不能以数字开头（参见 [api.py](/lib/bbpbcn/lib/api.py#L310) 中的正则表达式）。

### 从头创建类型定义和字段

虽然 bbpbcn 的许多工作流程都是围绕编辑解码器生成的类型定义展开的，但仍可以手动添加新字段或从头创建新的类型定义。
...
每个 protobuf 字段都需要有一个 `type` 属性，包含 bbpbcn 类型。`name` 属性是可选的，但建议用于可读性。
