...
<https://github.com/chunjie008/bbpbcn/blob/master/lib/README.md>
...
上方的列表显示可用于解码此消息的命名类型定义列表。选择一个将使用新类型重新解码消息。bbpbcn 会尝试记住该端点最后选择的类型。`New` 按钮将当前类型定义保存为新名称。
...
扩展的某些行为可以通过 `burp/bbpbcn/burp/user_funcs.py` 文件来更改。每个函数由扩展调用，以提供处理消息的替代方式：
