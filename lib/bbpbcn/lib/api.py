"""`bbpbcn.lib.api` 模块提供了用于解码和重新编码 protobuf 消息的高级函数。

大多数函数接收输入数据、类型定义和配置对象。

'message_type' 或类型定义（typedef）是 bbpbcn 特定的格式，
...
config 参数来自 `bbpbcn.lib.config` 的 Config 对象，
...
import json
import collections
import bbpbcn.lib.protofile
import bbpbcn.lib.types.length_delim
import bbpbcn.lib.types.type_maps
...
if six.PY3:
    ...
    if typing.TYPE_CHECKING:
        ...
        from bbpbcn.lib.pytypes import Message, TypeDefDict, FieldDefDict
...
def decode_message(buf, message_type=None, config=None):
    ...
    Args:
        ...
        message_type: 可选，用作解码基础的类型。允许自定义字段类型或名称。
            ...
            默认为空定义 '{}'。
        config: `bbpbcn.lib.config.Config` 对象，允许自定义
            线类型的默认类型，并包含 `known_types` 数组。
            如果未提供，默认为 `bbpbcn.lib.config.default`。
    Returns:
    ...
    value, typedef, _, _ = bbpbcn.lib.types.length_delim.decode_message(
...
def encode_message(value, message_type, config=None):
    ...
    return bytes(
        bbpbcn.lib.types.length_delim.encode_message(
...
def export_protofile(message_types, output_filename):
    ...
    """
    bbpbcn.lib.protofile.export_proto(
...
def import_protofile(input_filename, save_to_known=True, config=None):
    ...
    new_typedefs = bbpbcn.lib.protofile.import_proto(
...
def validate_typedef(typedef, old_typedef=None, config=None, _path=None):
    ...
    for field_number, field_typedef in typedef.items():
        ...
        for key, value in field_typedef.items():
            ...
            # 验证 type 值
            if key == "type":
                if value not in bbpbcn.lib.types.type_maps.WIRETYPES:
                    raise TypedefException(
    ...
    if old_typedef is not None:
        ...
        for field_number, value in old_typedef.items():
            wiretype_map[
                int(field_number)
            ] = bbpbcn.lib.types.type_maps.WIRETYPES[value["type"]]
