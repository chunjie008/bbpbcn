...
import bbpbcn.lib
from bbpbcn.lib.types import varint, wiretypes
...
from bbpbcn.lib.typedef import (
    TypeDef,
    MutableTypeDef,
    FieldDef,
    MutableFieldDef,
...
if six.PY3:
    ...
    if typing.TYPE_CHECKING:
        ...
        from bbpbcn.lib.pytypes import Message
...
def _has_protobuf_structure(buf):
    ...
    try:
        ...
        if wt == wiretypes.FIXED64 and data_end - pos < 9:
            return False
        return True
    except (DecoderException, bbpbcnException, IndexError, Exception):
        return False
...
def _encode_message_field(config, typedef, path, field_id, value):
    ...
    if isinstance(field_type, TypeDef):
        ...
        if field_type not in bbpbcn.lib.types.ENCODERS:
            raise TypedefException("Unknown type: %s" % field_type)
        field_encoder = bbpbcn.lib.types.ENCODERS[field_type]
    ...
    # 编码标签（tag）
    tag = encode_tag(
        int(field_number), bbpbcn.lib.types.WIRETYPES[field_type]
    )
...
def _decode_standard_field(wire_type, buffers, fielddef, config, field_path):
    ...
    for alt_type_id, field_type in fielddef.resolve_types(config, field_path).items():
        ...
        if (
            ...
            or bbpbcn.lib.types.WIRETYPES[field_type] != wire_type
        ):
        ...
        if field_type not in bbpbcn.lib.types.DECODERS:
            raise TypedefException(
                "Type %s does not have a decoder" % (field_type),
                path=field_path,
            )
        decoder = bbpbcn.lib.types.DECODERS[field_type]
        try:
    ...
    if field_outputs is None:
        ...
        default_decoder = bbpbcn.lib.types.DECODERS[field_type]
...
def _try_decode_lendelim_fields(buffers, fielddef, config, path):
    ...
    for target_type in binary_fallbacks:
        try:
            outputs = []
            decoder = bbpbcn.lib.types.DECODERS[target_type]
            for buf in buffers:
